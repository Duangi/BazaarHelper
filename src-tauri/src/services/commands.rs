...
pub fn clear_yolo_cache() -> Result<String, String> {
    crate::services::yolo_state::clear_cache();
...
pub async fn trigger_yolo_scan(app: tauri::AppHandle, useGpu: bool) -> Result<usize, String> {
    ...
    let result = (|| -> Result<usize, String> {
        ...
        let capture_monitor_by_cursor =
            || -> Result<(image::RgbaImage, i32, i32), String> {
        ...
        let detections = crate::monster_recognition::run_yolo_inference(&img, &model_path, use_gpu_flag)?;
        if crate::ABORT_YOLO.load(Ordering::SeqCst) {
            return Err("Aborted".into());
        }
